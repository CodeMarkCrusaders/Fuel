...
* графики P-T, V-M, ρ-γₛ, профиль сопла, состав продуктов сгорания;
  - **высота графиков** регулируется перетаскиваемым **сплиттером** под
    областью графиков (раньше — числовое поле);
  - **наложение профиля сопла** (контур r/r_кр на отдельной правой оси)
    включается флажком «Профиль сопла на графиках»;
  - **основная и дополнительная (минорная) сетка** включаются независимыми
    флажками и реально влияют на отрисовку;
  - раскладка графиков в 1 или 2 колонки (либо «Авто»);
* **панель «Параметры по сечениям»** при большом числе сечений
  прокручивается **горизонтально внутри своего контейнера** (колонки
  сохраняют читаемую ширину и не «распирают» вкладку);
* экспорт результатов в CSV (Amesim-совместимый формат);
* сохранение/загрузка конфигурации (JSON) через **системный проводник**
  (диалог выбора файла) — путь больше не задаётся жёстко;
