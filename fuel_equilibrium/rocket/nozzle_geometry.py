...
class NozzleGeometry:
    ...
    def map_area_ratios(
        ...
        supersonic_flags: Optional[Sequence[bool]] = None,
    ) -> Tuple[np.ndarray, np.ndarray]:
        """Сопоставляет каждому отношению площадей F/F_кр точку на контуре.

        Для каждого значения ``A/A_кр`` ищется осевая координата ``x`` на
        РЕАЛЬНОМ контуре сопла (этой геометрии), где локальное отношение
        площадей ``(r(x)/R_кр)²`` совпадает с заданным. Так профиль,
        наложенный на графики газодинамики, ТОЧНО повторяет контур выбранной
        геометрии (коническое / профилированное / RPA).

        Ветвь (дозвук слева / сверхзвук справа от горловины) выбирается по
        ``supersonic_flags``. Без флагов все сечения считаются сверхзвуковыми.
        Возвращает ``(x_m, r_m)`` numpy-массивов длины ``len(area_ratios)``.
        """
        xs, rs = self.as_xy_arrays()
        if xs.size == 0:
            n = len(area_ratios)
            return np.zeros(n), np.zeros(n)
        i_thr = int(np.argmin(rs))
        R_thr = float(self.R_throat_m) if self.R_throat_m else float(rs[i_thr])
        R_thr = R_thr if R_thr > 0 else 1.0
        ar_contour = (rs / R_thr) ** 2

        ars = np.asarray(area_ratios, dtype=float)
        n = ars.size
        if supersonic_flags is None:
            flags = np.ones(n, dtype=bool)
        else:
            flags = np.asarray(list(supersonic_flags), dtype=bool)
            if flags.size != n:
                flags = np.resize(flags, n)

        out_x = np.empty(n, dtype=float)
        out_r = np.empty(n, dtype=float)

        sub_ar = ar_contour[: i_thr + 1]
        sub_x = xs[: i_thr + 1]
        sup_ar = ar_contour[i_thr:]
        sup_x = xs[i_thr:]

        sub_order = np.argsort(sub_ar)
        sub_ar_s = sub_ar[sub_order]
        sub_x_s = sub_x[sub_order]
        sup_order = np.argsort(sup_ar)
        sup_ar_s = sup_ar[sup_order]
        sup_x_s = sup_x[sup_order]

        for k in range(n):
            a = float(ars[k])
            if not math.isfinite(a) or a < 1.0:
                a = 1.0
            if flags[k] and sup_ar_s.size >= 2:
                x_k = float(np.interp(a, sup_ar_s, sup_x_s))
            elif (not flags[k]) and sub_ar_s.size >= 2:
                x_k = float(np.interp(a, sub_ar_s, sub_x_s))
            else:
                j = int(np.argmin(np.abs(ar_contour - a)))
                x_k = float(xs[j])
            out_x[k] = x_k
            out_r[k] = float(np.interp(x_k, xs, rs))
        return out_x, out_r
