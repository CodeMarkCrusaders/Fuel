...
# папка с базами данных лежит на уровень выше — рядом с пакетом
# (core/equilibrium.py -> ../data/thermo.inp)
...
# Печать результатов вынесена в fuel_equilibrium.io.reporting.print_result.
# Импортируется лениво внутри функций, чтобы избежать циклов и потому что
# I/O — это не задача core-слоя.
...
def run_batch(
    ...
    try:
        ...
        if verbose:
            from ..io.reporting import print_result
