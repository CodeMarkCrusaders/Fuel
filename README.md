...
Поддерживается только **GUI-режим** (консольный интерфейс отключён):

```bash
python equilibrium.py
```

GUI запускает расчёт сопла для фиксированных условий:
- 4 сечения: вход в КС, начало сопла, критика, срез;
- `Pc = 10 МПа`, `Pe = 1 атм`;
- топливная пара: `LOX/LH2` (`O2(L)` + `H2(L)`);
- автоматический ускоренный поиск оптимального массового соотношения (максимум `Isp`).
